# Jeu de bataille spatial 

#touche pour le joueur 1 : Z,Q,S,D et la touche Ctrl coté gauche pour tirer 
#touche pour le joueur 2 : les flèches et la touche Ctrl coté droit pour tirer 
...
def main_menu():
    ...
    option1 = options_font.render('Start Game', True, WHITE)
    option2 = options_font.render('Exit', True, WHITE)
    title_pos = title.get_rect(center=(LONGUEUR//2, LARGEUR//2-50))
    option1_pos = option1.get_rect(center=(LONGUEUR//2, LARGEUR//2+50))
    option2_pos = option2.get_rect(center=(LONGUEUR//2, LARGEUR//2+100))
    menu_surface = pygame.Surface((LONGUEUR, LARGEUR))
    menu_surface.blit(title, title_pos)
    menu_surface.blit(option1, option1_pos)
    menu_surface.blit(option2, option2_pos)
    cursor_pos = option1_pos  # start with the first option selected
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP:
                    cursor_pos = option1_pos
                elif event.key == pygame.K_DOWN:
                    cursor_pos = option2_pos
                elif event.key == pygame.K_SPACE:
                    if cursor_pos == option1_pos:
                        return 'start'
                    elif cursor_pos == option2_pos:
                        pygame.quit()
                        sys.exit()
            elif event.type == pygame.MOUSEBUTTONDOWN:
                if option1_pos.collidepoint(event.pos):
                    return 'start'
                elif option2_pos.collidepoint(event.pos):
                    pygame.quit()
                    sys.exit()
        ...
        menu_surface.blit(option1, option1_pos)
        menu_surface.blit(option2, option2_pos)
        pygame.draw.rect(menu_surface, WHITE, cursor_pos, 5)
